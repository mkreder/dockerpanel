...
func calculatePort(id uint) string {
	if id < 10 {
		...
		log.Panic("We currently not support more than 1000 DBs")
	}
	return ""
}

func createDBconfigdir(name string) {
	if _, err := os.Stat("configs/db/Dockerfile"); os.IsNotExist(err) {
		log.Printf("Creating config files")
		srcFolder := "defaults/db"
		os.MkdirAll("configs/db", 0755)
		destFolder := "configs/"
		cpCmd := exec.Command("cp", "-rf", srcFolder, destFolder)
		err := cpCmd.Run()
		check(err)
		os.MkdirAll("data/db/"+name, 0755)
	}
}

func createPMAconfig() {
	...
	err := ioutil.WriteFile("configs/db/phpmyadmin/config.inc.php", []byte(config), 0644)
	check(err)
}

func createSQLuser(bd model.BD) {
	sql := ""
	var usuario model.DBUser
...
func correrContenedorBD(bd model.BD) {
	wd, _ := os.Getwd()
	cmdString := "cd configs/bd/" + bd.Nombre + "; docker stop dp-bd-" + bd.Nombre + "; docker rm dp-bd-" + bd.Nombre + "; docker run -d -p " + calculatePort(bd.ID) + ":3306" + " -v " + wd + "/configs/bd/" + bd.Nombre + "/conf:/conf:ro" + " -v " + wd + "/data/bd/" + bd.Nombre + ":/var/lib/mysql" + " --name dp-bd-" + bd.Nombre + " dp-img-bd"
	runCmd := exec.Command("/bin/sh", "-c", cmdString)
	var stderr bytes.Buffer
	var out bytes.Buffer
	runCmd.Stdout = &out
...
// RunDBWorker - Function to run this DB worker
func RunDBWorker() {
